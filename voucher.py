...
class voucher(orm.Model):
    ...
    def recompute_voucher_lines(self, cr, uid, ids, partner_id, journal_id, price,
        ...
        partner = partner_db.browse(cr, uid, partner_id)
        ...
        if res.get('value') and res['value'].get('line_cr_ids'):
            for vals in res['value']['line_cr_ids']:
                vals['supplier_invoice_number'] = False
        ...
        if partner.exact_supplier_number:
            if res.get('value') and res['value'].get('line_cr_ids'):
                res['value']['line_cr_ids'] = [x for x in res['value']['line_cr_ids'] if x['supplier_invoice_number'] == context['default_name']]
            if res.get('value') and res['value'].get('line_dr_ids'):
                res['value']['line_dr_ids'] = [x for x in res['value']['line_dr_ids'] if x['supplier_invoice_number'] == context['default_name']]
        return res
